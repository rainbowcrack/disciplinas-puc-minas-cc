...
#define MAX_LISTA_SIZE 200 // Maximo de shows na lista manipulada
...
void removerInicio(SHOW lista[], int *tamanho) {
    ...
    if(*tamanho > 0) (*tamanho)--; // decrementa
    if (*tamanho > 0 && *tamanho < MAX_LISTA_SIZE) {
         memset(&lista[*tamanho], 0, sizeof(SHOW)); 
    } else if (*tamanho == 0) { // preenche com o tamanho da lista, memset() preenche
...
int main() {
    ...
    if (total_csv == 0 && MAX_SHOWS_CSV > 0) { 
    }
    ...
    if (scanf("%d", &num_comandos) != 1) {
        // fprintf(stderr, "Erro ao ler numero de comandos.\n");
        // return 1;
    }
