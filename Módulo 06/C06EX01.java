...
public class C06EX01 {
    public static void main(String[] args) {
        ...
        if (x == 4) {
            fx = 0; // se valor eh 4 retorna em 0
        } else {
            if (x < 4) // se valor menor que 4, retorna
                fx = ((5 * x) + 3) / Math.sqrt(16 - Math.pow(x, 2));
            else // senao cumpridas as outras condicoes, retorna
                fx = ((5 * x) + 3) / Math.sqrt(Math.pow(x, 2) - 16);
        }
        System.out.print("F(x) = " + fx); // output fora da condicional
        teclado.close();
    }
}
